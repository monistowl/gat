...
\large{Comprehensive Technical Reference for Doctoral-Level Engineers}}
...
\date{November 2024 --- Draft 6 (Expanded)}
...
\centering
\begin{tikzpicture}[
    crate/.style={rectangle, draw, rounded corners, minimum width=2.5cm, minimum height=0.8cm, align=center, font=\small\ttfamily},
    ...
    % Solver backends
    \node[crate, fill=yellow!20] (ipopt) at (-4,-3) {gat-ipopt};
    \node[crate, fill=yellow!20] (cbc) at (-2,-3) {gat-cbc};
    \node[crate, fill=yellow!20] (clp) at (0,-3) {gat-clp};

    % Application layer
    \node[crate, fill=red!20] (cli) at (3,-1.5) {gat-cli};
    \node[crate, fill=red!20] (tui) at (5,-1.5) {gat-tui};

    % Specialized
    \node[crate, fill=purple!20] (ts) at (2,-3) {gat-ts};
    \node[crate, fill=purple!20] (dist) at (4,-3) {gat-dist};
...
\textbf{Critical bug fix (November 2024):} The to-side thermal constraint requires careful application of the chain rule for $\theta_{\text{diff}} = \theta_j - \theta_i + \phi$:
