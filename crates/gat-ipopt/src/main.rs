...
fn solve_with_ipopt(problem: &ProblemBatch) -> Result<SolutionBatch> {
    ...
    for i in 0..n_bus {
        temp_network.graph.add_node(Node::Bus(gat_core::Bus {
            ...
            base_kv: gat_core::Kilovolts(138.0), // Default, not used for Y-bus construction
            voltage_pu: gat_core::PerUnit(1.0),
            angle_rad: gat_core::Radians(0.0),
            vmin_pu: Some(gat_core::PerUnit(problem.bus_v_min[i])),
            vmax_pu: Some(gat_core::PerUnit(problem.bus_v_max[i])),
            area_id: None,
            zone_id: None,
        }));
    }
    ...
    for i in 0..problem.branch_id.len() {
        ...
        temp_network.graph.add_edge(
            ...
            Edge::Branch(Branch {
                ...
                phase_shift: gat_core::Radians(problem.branch_shift[i]),
                charging_b: gat_core::PerUnit(problem.branch_b[i]),
                s_max: Some(gat_core::MegavoltAmperes(problem.branch_rate[i])),
                rating_a: Some(gat_core::MegavoltAmperes(problem.branch_rate[i])),
                rating_b: None,
                rating_c: None,
                status: true,
                angle_min: None,
    ...
    for i in 0..n_gen {
        ...
        generators.push(GenData {
            ...
            pmin: problem.gen_p_min[i],
            pmax: problem.gen_p_max[i],
            qmin: problem.gen_q_min[i],
            qmax: problem.gen_q_max[i],
            cost_coeffs,
            cost_model,
