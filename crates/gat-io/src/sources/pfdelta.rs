...
pub struct PFDeltaInstance {
    ...
    /// Import diagnostics (warnings about silent defaults, parsing issues)
...
pub fn load_pfdelta_instance(
    ...
    apply_solved_dispatch(&mut network, &data, &mut diagnostics)?;
...
fn apply_solved_dispatch(network: &mut Network, data: &Value, diagnostics: &mut ImportDiagnostics) -> Result<()> {
    ...
    for (gen_idx_str, gen_data) in gen_solutions {
        let gen_idx: usize = match gen_idx_str.parse() {
            Ok(idx) => idx,
            Err(_) => {
                diagnostics.add_warning(
                    "parse",
                    &format!("Generator solution key '{}' is not a valid index, skipping", gen_idx_str),
                );
                continue;
            }
        };
    ...
    if let Some(bus_solutions) = sol["bus"].as_object() {
        for (bus_idx_str, bus_data) in bus_solutions {
            let bus_idx: usize = match bus_idx_str.parse() {
                Ok(idx) => idx,
                Err(_) => {
                    diagnostics.add_warning(
                        "parse",
                        &format!("Bus solution key '{}' is not a valid index, skipping", bus_idx_str),
                    );
                    continue;
                }
            };
        ...
        for (gen_idx_str, gen_data) in network_gens {
            let gen_idx: usize = match gen_idx_str.parse() {
                ...
                Err(_) => {
                    diagnostics.add_warning(
                        "parse",
                        &format!("Network generator key '{}' is not a valid index, skipping", gen_idx_str),
                    );
                    continue;
                }
            };
            let gen_bus = match gen_data["gen_bus"]
                .as_u64()
                .or_else(|| gen_data["bus"].as_u64())
            {
                Some(bus) => bus as usize,
                None => {
                    diagnostics.add_warning(
                        "parse",
                        &format!("Generator {} has no gen_bus/bus field, defaulting to bus 0", gen_idx),
                    );
                    0
                }
            };
