...
use gat_core::{
    Branch, BranchId, Bus, BusId, Edge, Gen, GenId, Load, LoadId, Network, Node, Shunt, ShuntId,
};
        ...
        let gen_idx: usize = match gen_idx_str.parse() {
            ...
            Err(_) => {
                diagnostics.add_warning(
                    "parse",
                    &format!(
                        "Generator solution key '{}' is not a valid index, skipping",
                        gen_idx_str
                    ),
                );
                continue;
            ...
            let bus_idx: usize = match bus_idx_str.parse() {
                ...
                Err(_) => {
                    diagnostics.add_warning(
                        "parse",
                        &format!(
                            "Bus solution key '{}' is not a valid index, skipping",
                            bus_idx_str
                        ),
                    );
                    continue;
            ...
            let gen_idx: usize = match gen_idx_str.parse() {
                ...
                Err(_) => {
                    diagnostics.add_warning(
                        "parse",
                        &format!(
                            "Network generator key '{}' is not a valid index, skipping",
                            gen_idx_str
                        ),
                    );
                    continue;
                ...
                .or_else(|| gen_data["bus"].as_u64())
            {
                ...
                None => {
                    diagnostics.add_warning(
                        "parse",
                        &format!(
                            "Generator {} has no gen_bus/bus field, defaulting to bus 0",
                            gen_idx
                        ),
                    );
                    0
