...
impl<'a> NetworkBuilder<'a> {
    ...
    pub fn add_bus(&mut self, input: BusInput) -> NodeIndex {
        ...
        let node_idx = self.network.graph.add_node(Node::Bus(Bus {
            id: bus_id,
            name,
            base_kv: gat_core::Kilovolts(input.voltage_kv),
            voltage_pu: gat_core::PerUnit(input.voltage_pu.unwrap_or(1.0)),
            angle_rad: gat_core::Radians(input.angle_rad.unwrap_or(0.0)),
            vmin_pu: input.vmin_pu.map(gat_core::PerUnit),
            vmax_pu: input.vmax_pu.map(gat_core::PerUnit),
    ...
    pub fn add_load(&mut self, input: LoadInput) -> AddResult {
        ...
        self.network.graph.add_node(Node::Load(Load {
            ...
            active_power: gat_core::Megawatts(input.active_power_mw),
            reactive_power: gat_core::Megavars(input.reactive_power_mvar),
        }));
    ...
    pub fn add_gen(&mut self, input: GenInput) -> AddResult {
        ...
        self.network.graph.add_node(Node::Gen(Gen {
            ...
            name,
            bus: BusId::new(input.bus_id),
            active_power: gat_core::Megawatts(input.pg),
            reactive_power: gat_core::Megavars(input.qg),
            pmin: gat_core::Megawatts(input.pmin),
            pmax: gat_core::Megawatts(input.pmax),
            qmin: gat_core::Megavars(input.qmin),
            qmax: gat_core::Megavars(input.qmax),
            voltage_setpoint: input.voltage_setpoint_pu.map(gat_core::PerUnit),
            mbase: input.mbase_mva.map(gat_core::MegavoltAmperes),
    ...
    pub fn add_branch(&mut self, input: BranchInput) -> AddResult {
        ...
        let branch = Branch {
            ...
            phase_shift: gat_core::Radians(input.phase_shift_rad),
            charging_b: gat_core::PerUnit(input.charging_b),
            s_max: input.rate_mva.map(gat_core::MegavoltAmperes),
            status: true,
            rating_a: input.rate_mva.map(gat_core::MegavoltAmperes),
            rating_b: input.rating_b_mva.map(gat_core::MegavoltAmperes),
            rating_c: input.rating_c_mva.map(gat_core::MegavoltAmperes),
            angle_min: input.angle_min_rad.map(gat_core::Radians),
            angle_max: input.angle_max_rad.map(gat_core::Radians),
            element_type,
