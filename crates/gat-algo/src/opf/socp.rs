...
/// Initial point for SOCP from DC-OPF warm-start.
///
...
pub struct SocpInitialPoint {
    ...
    pub vm_squared: Vec<f64>,
    /// Voltage angles in radians (from DC-OPF)
    pub va: Vec<f64>,
    /// Generator real power in per-unit
    pub pg: Vec<f64>,
    /// Generator reactive power in per-unit (estimated from power factor)
    pub qg: Vec<f64>,
    /// Squared current magnitudes on branches
    pub ell: Vec<f64>,
    /// Branch real power flow in per-unit
    pub p_flow: Vec<f64>,
    /// Branch reactive power flow in per-unit
    pub q_flow: Vec<f64>,
}
...
impl SocpInitialPoint {
    ...
    pub fn flat_start(n_bus: usize, n_gen: usize, n_branch: usize) -> Self {
        Self {
            vm_squared: vec![1.0; n_bus],
            va: vec![0.0; n_bus],
            pg: vec![0.0; n_gen],
            qg: vec![0.0; n_gen],
            ell: vec![0.01; n_branch], // Small positive to avoid SOC boundary
            p_flow: vec![0.0; n_branch],
            q_flow: vec![0.0; n_branch],
        }
    }
}

/// Create SOCP initial point from DC-OPF solution.
///
/// This function extracts warm-start data from a DC-OPF solution:
/// - Voltage angles: directly from DC solution
/// - Voltage magnitudes: set to 1.0 (DC assumption)
/// - Generator Pg: directly from DC solution
/// - Generator Qg: estimated using typical power factor (0.9 lagging)
/// - Branch currents: computed from DC flows
///
/// # Arguments
/// * `dc_warm` - Warm-start data extracted from DC-OPF solution
/// * `bus_names` - Ordered list of bus names (for index mapping)
/// * `gen_names` - Ordered list of generator names
/// * `gen_bus_map` - Map from generator name to bus name
/// * `branch_data` - Branch information for flow estimation
///
/// # Returns
...
pub fn warm_start_from_dc(
    ...
    bus_names: &[String],
    gen_names: &[String],
    gen_bus_map: &std::collections::HashMap<String, String>,
    n_branch: usize,
) -> SocpInitialPoint {
    ...
    // Voltage angles: from DC solution (already in radians for DC)
    let va: Vec<f64> = bus_names
        .iter()
        .map(|name| dc_warm.bus_angles.get(name).copied().unwrap_or(0.0))
        .collect();

    // Generator real power: from DC solution, convert to per-unit
    let pg: Vec<f64> = gen_names
        .iter()
        .map(|name| dc_warm.generator_p.get(name).copied().unwrap_or(0.0) / BASE_MVA)
        .collect();

    // Generator reactive power: estimate from power factor
    // Typical generators operate at 0.9 power factor lagging
    // Q = P × tan(acos(0.9)) ≈ P × 0.484
    let qg: Vec<f64> = pg.iter().map(|&p| p * 0.484).collect();

    // Branch currents and flows: estimate from power balance
    // For initial point, use small positive values to stay interior to SOC
    let ell = vec![0.01; n_branch];
    let p_flow = vec![0.0; n_branch];
    let q_flow = vec![0.0; n_branch];

    let _ = gen_bus_map; // Will be used for more sophisticated Qg estimation
