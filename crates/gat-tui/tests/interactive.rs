...
fn test_interactive_keypresses_and_responses() {
    ...
    println!(
        ...
        initial_render.len(),
        initial_render.lines().count()
    );

    assert_eq!(
        app.active_menu_label(),
        Some("Dashboard"),
        "Initial pane should be Dashboard"
    );
    assert!(
        initial_render.contains("Dashboard"),
        "Dashboard content missing"
    );
    assert!(
        initial_render.contains("[*1]"),
        "Dashboard indicator missing"
    );
    ...
    println!(
        "Output length: {} chars, {} lines\n",
        render_3.len(),
        render_3.lines().count()
    );

    assert_eq!(
        app.active_menu_label(),
        Some("Datasets"),
        "Should switch to Datasets"
    );
    assert!(render_3.contains("[*3]"), "Datasets indicator missing");
    assert!(
        render_3.contains("Data catalog"),
        "Datasets content missing"
    );
    assert_ne!(
        render_2, render_3,
        "Render should change when switching panes"
    ...
    println!(
        "Output length: {} chars, {} lines\n",
        render_4.len(),
        render_4.lines().count()
    );

    assert_eq!(
        app.active_menu_label(),
        Some("Pipeline"),
        "Should switch to Pipeline"
    );
    assert!(render_4.contains("[*4]"), "Pipeline indicator missing");
    assert!(
        render_4.contains("Pipeline composer"),
        "Pipeline content missing"
    );
    assert_ne!(
        render_3, render_4,
        "Render should change when switching panes"
    ...
    assert_eq!(
        initial_render, render_6,
        "Should be identical to initial render"
    );
...
fn test_output_format_is_structured() {
    ...
    assert!(
        lines.len() > 10,
        "Should have multiple lines of output, not a single dump"
    );
    assert!(short_lines > 0, "Should have some short lines (structure)");
    assert!(
        medium_lines > 0 || long_lines > 0,
...
fn test_realistic_interaction_workflow() {
    ...
    for (key, expected_label, expected_content) in navigation {
        ...
        println!(
            ...
            render.contains(expected_content)
        );

        assert_eq!(
            label, expected_label,
            "Label mismatch after pressing '{}'",
            key
        );
        assert!(
            render.contains(expected_content),
    ...
    assert_eq!(
        renders[0], renders[4],
        "Initial and final Dashboard renders should be identical"
    );
    assert_ne!(
        renders[0], renders[1],
        "Dashboard and Operations should be different"
    );
    assert_ne!(
        renders[1], renders[2],
        "Operations and Pipeline should be different"
    );
    assert_ne!(
        renders[2], renders[3],
        "Pipeline and Commands should be different"
    );
