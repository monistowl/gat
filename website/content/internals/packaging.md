...
GAT 0.5.4 introduces a modular installation system with two main approaches: **modular component selection** (on-demand installation) and **bundle variants** (pre-packaged tarballs). Both support binary-first delivery with fallback to source builds.
...
```bash
# CLI only (default)
curl -fsSL https://raw.githubusercontent.com/monistowl/gat/v0.5.4/scripts/install-modular.sh \
  | bash

# CLI + TUI
GAT_COMPONENTS=cli,tui \
  bash <(curl -fsSL https://raw.githubusercontent.com/monistowl/gat/v0.5.4/scripts/install-modular.sh)
...
curl -fsSL \
  https://github.com/monistowl/gat/releases/download/v0.5.4/gat-0.5.4-linux-x86_64-full.tar.gz \
  | tar xz
cd gat-0.5.4-linux-x86_64-full && ./install.sh

# Headless variant (CLI only, minimal footprint)
curl -fsSL \
  https://github.com/monistowl/gat/releases/download/v0.5.4/gat-0.5.4-linux-x86_64-headless.tar.gz \
  | tar xz
cd gat-0.5.4-linux-x86_64-headless && ./install.sh --variant headless
```
...
```
dist/
├── gat-0.5.4-linux-x86_64-headless.tar.gz
├── gat-0.5.4-linux-x86_64-analyst.tar.gz
├── gat-0.5.4-linux-x86_64-full.tar.gz
├── gat-0.5.4-macos-x86_64-full.tar.gz
├── gat-0.5.4-macos-arm64-full.tar.gz
└── ...
```
...
```
https://github.com/monistowl/gat/releases/download/v0.5.4/gat-0.5.4-<os>-<arch>-<variant>.tar.gz
```
...
v0.1 users upgrading to v0.5.4:
