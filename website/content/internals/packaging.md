...
```bash
# CLI only (default)
curl -fsSL https://raw.githubusercontent.com/monistowl/gat/v0.5.7/scripts/install-modular.sh \
  | bash

# CLI + TUI
GAT_COMPONENTS=cli,tui \
  bash <(curl -fsSL https://raw.githubusercontent.com/monistowl/gat/v0.5.7/scripts/install-modular.sh)
...
curl -fsSL \
  https://github.com/monistowl/gat/releases/download/v0.5.7/gat-0.5.7-linux-x86_64-full.tar.gz \
  | tar xz
cd gat-0.5.7-linux-x86_64-full && ./install.sh

# Headless variant (CLI only, minimal footprint)
curl -fsSL \
  https://github.com/monistowl/gat/releases/download/v0.5.7/gat-0.5.7-linux-x86_64-headless.tar.gz \
  | tar xz
cd gat-0.5.7-linux-x86_64-headless && ./install.sh --variant headless
```
